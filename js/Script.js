function main()
{
    var f = document.getElementById("fun").value;
    var s = document.getElementById("lims").value;
    var i = document.getElementById("limi").value;
    
    if((f.indexOf('e') > -1) && !(f.indexOf("sec") > -1))
        f = f.replace(/e/gi, "2.718281828");
    
    if((s.indexOf("pi") > -1))
        s = s.replace(/pi/gi, "180");
    
    if((i.indexOf("pi") > -1))
        i = i.replace(/pi/gi, "180");
    
    var fun = Parser.parse(f);
    var sup = Parser.parse(s);
    var inf = Parser.parse(i);
    
    s = sup.evaluate({x: 1});
    i = inf.evaluate({x: 1});
    
    if((f.indexOf("sin") > -1) || (f.indexOf("cos") > -1) || (f.indexOf("tan") > -1) || (f.indexOf("ctan") > -1) || (f.indexOf("sec") > -1) || (f.indexOf("csc") > -1))
    {
        s *= 0.0174533;
        i *= 0.0174533;       
    }
    
    var res = fun.evaluate({ x: s });
    res += fun.evaluate({ x : i });
    
    for(var c = 1; c < 500000; c++)
    {
        res += 2 * fun.evaluate({x : (i + ( (c * (s - i) ) / 500000) )});       
    }
